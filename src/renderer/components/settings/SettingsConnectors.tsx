...
export function SettingsConnectors({ isActive }: { isActive: boolean }) {
  ...
  // OAuth: per-server "authenticated" flags and the server currently logging in.
  ...
  const loadDisabledTools = useCallback(async () => {
    try {
      const loaded = await window.electronAPI.mcp.getDisabledTools();
      setDisabledTools(loaded || []);
    } catch (err) {
      console.error('Failed to load disabled tools:', err);
    }
  }, []);
  ...
  const handleToggleTool = useCallback(
    async (toolName: string, enabled: boolean) => {
      ...
      setDisabledTools((prev) => {
        ...
        if (enabled) set.delete(lower);
        else set.add(lower);
        return [...set];
      });
      try {
        await window.electronAPI.mcp.setToolEnabled({ toolName, enabled });
      } catch (err) {
        console.error('Failed to toggle tool:', err);
        void loadDisabledTools(); // revert to source of truth on error
      }
    },
    [loadDisabledTools]
  );
  ...
  }, [loadPresets, loadServers, loadStatuses, loadTools, loadDisabledTools]);
  ...
  // Load OAuth status for HTTP/SSE servers whenever the server list changes.
  useEffect(() => {
    if (!isElectron || !isActive) {
      return;
    }
    const httpServers = servers.filter(
      (s) => s.type === 'sse' || s.type === 'streamable-http'
    );
    ...
    void (async () => {
      const entries = await Promise.all(
        httpServers.map(async (s) => {
          try {
            const r = await window.electronAPI.mcp.getOAuthStatus(s.id);
            return [s.id, r.authenticated] as const;
          } catch {
  ...
  async function handleAuthenticate(server: MCPServerConfig) {
    setError('');
    setAuthenticatingId(server.id);
    try {
      // Authenticating implies the user wants to use this connector → enable it.
      const result = await window.electronAPI.mcp.authenticate({ ...server, enabled: true });
      if (!result.success) {
        setError(result.error || t('mcp.authFailed'));
        return;
      }
      setOauthStatus((prev) => ({ ...prev, [server.id]: true }));
      await loadAll();
    } catch (err) {
      setError(err instanceof Error ? err.message : t('mcp.authFailed'));
    } finally {
      setAuthenticatingId(null);
    }
  }

  async function handleClearOAuth(server: MCPServerConfig) {
    if (!confirm(t('mcp.signOutConfirm'))) return;
    setError('');
    try {
      await window.electronAPI.mcp.clearOAuth(server.id);
            ...
            servers.map((server) => {
              ...
              return (
                <ServerCard
                  ...
                  isLoading={isLoading}
                  oauthAuthenticated={oauthStatus[server.id] ?? false}
                  isAuthenticating={authenticatingId === server.id}
                  onAuthenticate={() => handleAuthenticate(server)}
                  onClearOAuth={() => handleClearOAuth(server)}
                />
              );
            })
          )}
...
function ServerCard({
  ...
  // OAuth applies only to remote servers that aren't already using basic auth
  // (credentials embedded in the URL or a manual Authorization header).
  const isRemote = server.type === 'sse' || server.type === 'streamable-http';
  const usesBasicAuth =
    !!(server.headers &&
      Object.keys(server.headers).some((k) => k.toLowerCase() === 'authorization')) ||
    /\/\/[^/@]+:[^/@]+@/.test(server.url || '');
  const showOAuth = isRemote && !usesBasicAuth;
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1 min-w-0">
            ...
            <div className="text-sm text-text-muted space-y-1 ml-6 min-w-0">
              ...
              {/* OAuth authentication (one-time login at assignment) */}
              {showOAuth && (
                <div className="mt-2 space-y-1">
                  {oauthAuthenticated ? (
                    <div className="flex items-center gap-2">
                      <span className="inline-flex items-center gap-1 text-xs px-2 py-1 rounded-md bg-success/10 text-success w-fit">
                        <ShieldCheck className="w-3.5 h-3.5" />
                        {t('mcp.oauthAuthenticated')}
                      </span>
                      <button
                        type="button"
                        onClick={onClearOAuth}
                        className="text-xs text-text-muted hover:text-error transition-colors inline-flex items-center gap-1"
                        title={t('mcp.signOut')}
                      >
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-md bg-accent/10 text-accent hover:bg-accent/20 transition-colors disabled:opacity-60 w-fit"
                    >
                      {isAuthenticating ? (
                      ...
                      {isAuthenticating ? t('mcp.authenticating') : t('mcp.authenticate')}
                    </button>
                  )}
                  {isAuthenticating && (
                    <div className="text-xs text-text-muted">{t('mcp.authWaitingBrowser')}</div>
                  )}
                  {!oauthAuthenticated && !isAuthenticating && (
                    <div className="text-xs text-text-muted">{t('mcp.authHint')}</div>
                  )}
                </div>
              )}
                ...
                <div className="mt-3 p-3 rounded-lg bg-surface-muted border border-border">
                  ...
                  <div className="grid grid-cols-1 gap-2 max-h-48 overflow-y-auto">
                    {tools.map((tool, idx) => {
                      ...
                      const isEnabled = !disabledToolSet.has(tool.name.toLowerCase());
                      return (
                        <div
                          key={idx}
                          className={`flex items-start justify-between gap-2 px-2 py-1.5 rounded bg-background border border-border text-xs transition-opacity ${
                            isEnabled ? 'text-text-secondary' : 'text-text-muted opacity-60'
                          }`}
                        ...
                        >
                          ...
                          <button
                            ...
                            onClick={() => onToggleTool(tool.name, !isEnabled)}
                            ...
                            className={`relative mt-0.5 inline-flex h-5 w-9 flex-shrink-0 items-center rounded-full transition-colors ${
                              isEnabled ? 'bg-accent' : 'bg-text-muted/30'
                            }`}
                          >
                            <span
...
function ServerForm({
  ...
  const [autoApproveTools, setAutoApproveTools] = useState(server?.autoApproveTools ?? false);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-2">
        ...
        />
        <label htmlFor="autoApproveTools" className="text-sm text-text-primary">
          {t('mcp.autoApprove')}
          <span className="block text-xs text-text-muted">{t('mcp.autoApproveHint')}</span>
        </label>
      </div>
